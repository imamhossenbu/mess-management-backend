...
let EmailService = EmailService_1 = class EmailService {
    ...
    async sendBillEmail(user, billAmount, dueDate, month, details) {
    ...
    `;
        ...
        const host = this.config.get("SMTP_HOST");
        if (!host) {
            this.logger.warn(`SMTP is not configured; bill email for ${user.email} was not sent.`);
            return { sent: false };
        }
        try {
            const transporter = nodemailer.createTransport({
            ...
            const errorMessage = error instanceof Error ? error.message : "Unknown error occurred";
            this.logger.error(`Failed to send bill email to ${user.email}:`, errorMessage);
            return { sent: false, error: errorMessage };
        }
    }
